...
# Create lists of different as variables
...
def merge_files(file_list: list, outfile: str):
    ...
    gdfs = [gpd.read_file(f, use_arrow=True) for f in file_list]
    ...
    if file_list == imf:
        gdf = merged.to_crs(epsg='4326')
        gdf.to_parquet(f'{parquet_path}/bbox.parquet', 
                            compression='zstd',
                            write_covering_bbox=True)
